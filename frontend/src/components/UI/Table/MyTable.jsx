...
const MyTable = ({ fulldata, tabName, role }) => {
	...
	useEffect(() => {
		...
		if (Object.keys(expired).length > 0) {
			if (hasDanger) {
				setDangerDate("❌ Внимание! У вас есть програмное обеспечение с истекшими лицензиями.");
			}
			if (hasWarning) {
				setWarningDate("⚠️ Внимание! У вас есть програмное обеспечение с лицензиями, которые истекут в течение 30 дней.");
			}
		}
