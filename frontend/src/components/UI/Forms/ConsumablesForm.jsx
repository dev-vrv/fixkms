...
const ConsumablesForm = ({ onClose }) => {
  ...
  const handleSubmit = async () => {
    try {
      const { status, data } = await fetchForm("assets/consumables", "post", formData);  // Используем fetchForm
  ...
  const adminFields = [
    "Дата_Приобретения", "Количество", "Сумма", "Заправлен", "Дата_последней_заправки", "Поставщик",
    ...
    "IP_Адрес", "Домен", "MAC_Адрес", "Гарантия_До", "Тип", "Модель", "Номер_Партии",
  ];

  // Поля, доступные для обычного сотрудника
  const standardFields = [
    "Сотрудник", "Количество", "Сумма", "Серийный_Номер_Связанного_Объекта", "Инв_Номер_Бухгалтерии_Связанного_Объекта",
  ];
